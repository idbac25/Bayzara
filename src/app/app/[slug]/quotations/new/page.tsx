...
export default async function NewQuotationPage({ params, searchParams }: Props) {
  ...
  const { data: nextNumber } = await supabase
    .rpc('get_next_document_number', { p_business_id: business?.id, p_type: 'quotation' })
  ...
  return (
    <InvoiceForm
      ...
      clients={clients ?? []}
      inventory={inventory ?? []}
      nextDocNumber={nextNumber ?? 'QUO-0001'}
